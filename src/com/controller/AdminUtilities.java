	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getParameter("AdminAction").equals("FilterTrainers")){
			...
			try {
				...
				System.out.println(dbService.getSkillsByCId(c.getCourseId()));
				...
				for(Skill s :skillSet){
					...
					System.out.println("trainer by date");
					System.out.println((trainersListByDate.get(0).getTrainerId()));
					...
					System.out.println("trainer by skill");
					System.out.println(trainersListBySkill.get(0).getTrainerId());
					
					hsn.setAttribute("trainerListBySkill", trainersListBySkill);
					...
					trainerList.retainAll(trainersListBySkill);
					
					System.out.println("Intersection" + trainerList);
					programMap.put(s, trainerList);
					
				}
				
				Set<Skill> keySet = programMap.keySet();
				
				for(Skill s :keySet)
					System.out.println(programMap.get(s));
		...
		//If admin wants to allocate trainer(s) to the selected program for a particular date period.
			...
			System.out.println("Hey! Allocate trainers");
			try {
				...
				System.out.println("Allocated trainers" + trainerIdList);
		...
		//If admin wants to filter SME's by program to be taught during a particular date period.
			...
			try {
				...
				for(Skill s :skillSet){
					...
					SMEListByDate.retainAll(SMEListBySkill);
					ArrayList<SME> SMEList = (ArrayList<SME>) (SMEListByDate);
